...
for (var i = 0; i < 100; i++) {    
    ...
    var color1 = Math.ceil((Math.random() * 255) + 1);  // number between 1 and 255
    var color2 = Math.ceil((Math.random() * 5) + 1);
    var color3 = Math.ceil((Math.random() * 10) + 1);

    console.log(`fillColor:  + ${color1} | ${color2} | ${color3}`);

    var fillColor = "rgba("+ color1 +", "+ color2 +", "+ color3 +", 0." + i + ")";
